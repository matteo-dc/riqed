...
//    jvert_t L_proj(vvvprop_t(vvprop_t(vprop_t(prop_t::Zero(),5),nmr),nmr),njacks);  // L_proj has 5 components: S,V,P,A,T
    ...
    valarray<jproj_t> jG(vvvvd_t(vvvd_t(vvd_t(vd_t(0.0,5),nmr),nmr),njacks),4);
    valarray<jproj_t> jG_LO_and_EM(vvvvd_t(vvvd_t(vvd_t(vd_t(0.0,5),nmr),nmr),njacks),2);
            ...
            for(int mr_bw=0;mr_bw<nmr;mr_bw++)
                for(int igam=0;igam<16;igam++)
                    for(int k=0;k<4;k++)
                    {
                        ...
                        if(igam==0) /*L_proj[ijack][mr_fw][mr_bw][0]*/jG[k][ijack][mr_fw][mr_bw][0]=(jLambda[k][ijack][mr_fw][mr_bw][0]*P[0]).trace().real()/12.0;
                        if(igam>0 and igam<5) jG[k][ijack][mr_fw][mr_bw][1]+=(jLambda[k][ijack][mr_fw][mr_bw][igam]*P[igam]).trace().real()/12.0;
                        if(igam==5) jG[k][ijack][mr_fw][mr_bw][2]=(jLambda[k][ijack][mr_fw][mr_bw][5]*P[5]).trace().real()/12.0;
                        if(igam>5 and igam<10) jG[k][ijack][mr_fw][mr_bw][3]+=(jLambda[k][ijack][mr_fw][mr_bw][igam]*P[igam]).trace().real()/12.0;
                        if(igam>=10 and igam<16) jG[k][ijack][mr_fw][mr_bw][4]+=(jLambda[k][ijack][mr_fw][mr_bw][igam]*P[igam]).trace().real()/12.0;
