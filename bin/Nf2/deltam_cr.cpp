...
#pragma omp parallel for collapse(3)
    ...
    for(int m_fw=0;m_fw<nm;m_fw++)
        for(int m_bw=0;m_bw<nm;m_bw++)
            jdeltam_cr[m_fw][m_bw] = fit_par_jackknife(coord, error[m_fw][m_bw], deltam_cr_corr[m_fw][m_bw], t_min, t_max);
...
int main(int narg,char **arg)
{
    ...
    if (outfile.is_open())
    {
        for(int ijack=0;ijack<njacks;ijack++)
            for(int m_fw=0;m_fw<nm;m_fw++)
                for(int m_bw=0;m_bw<nm;m_bw++)
                    outfile.write((char*) &deltam_cr_array[ijack][m_fw][m_bw],sizeof(double));
